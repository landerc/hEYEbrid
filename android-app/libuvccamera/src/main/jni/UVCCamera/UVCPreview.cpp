...
double now_ms(void) {
    ...
    struct timespec res;
    clock_gettime(CLOCK_REALTIME, &res);//CLOCK_REALTIME
    double part1 = 1000 * res.tv_sec;
    int part2 = res.tv_nsec / 1000;
    //return (double) part1 + part2;
    return trunc(1000.0 * res.tv_sec + (double) res.tv_nsec / 1e6);
}
...
void UVCPreview::do_capture_callback(JNIEnv *env, uvc_frame_t *frame) {
	...
	if (LIKELY(frame)) {
		...
		if (mFrameCallbackObj) {
			...
			if (mFrameCallbackObj) {
				env->CallVoidMethod(mFrameCallbackObj, iframecallback_fields.onFrame, buf, ts);
			} else {
                LOGW("callback is gone!");
            }
			env->ExceptionClear();
