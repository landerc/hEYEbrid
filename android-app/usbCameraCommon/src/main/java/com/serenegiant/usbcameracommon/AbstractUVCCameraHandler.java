...
public abstract class AbstractUVCCameraHandler extends Handler {
	...
	private static final int MSG_FRAME_CAP = 10;
	private static final int MSG_FRAME_CAP_RELEASE = 11;
	...
	public void startRecording(final String prefix) {
		checkReleased();
		sendMessage(obtainMessage(MSG_CAPTURE_START, prefix));
	}

	public void startGrabbing() {
		checkReleased();
		sendEmptyMessage(MSG_FRAME_CAP);
	}

	public void stopGrabbing() {
		checkReleased();
		sendEmptyMessage(MSG_FRAME_CAP_RELEASE);
	}
	...
	public void handleMessage(final Message msg) {
		...
		case MSG_RELEASE:
			...
			break;
		case MSG_FRAME_CAP:
			thread.handleFrameCallback(true);
			break;
		case MSG_FRAME_CAP_RELEASE:
			thread.handleFrameCallback(false);
			break;
	...
	static final class CameraThread extends Thread {
		...
		private final IFrameCallback mIFrameCallback = new IFrameCallback() {
			@Override
			public void onFrame(final ByteBuffer frame, double ts) {

                if (mFrameProcessingCallback != null) {
                    mFrameProcessingCallback.onFrame(frame, ts);
                }
		...
		private IFrameCallback mFrameProcessingCallback;
		...
		CameraThread(final Class<? extends AbstractUVCCameraHandler> clazz,
					 final Activity parent, final CameraViewInterface cameraView,
					 final int encoderType, final int width, final int height, final int format,
					 final float bandwidthFactor, IFrameCallback callback) {
			...
			mFrameProcessingCallback = callback;
		}
		...
		public void handleCaptureStill(final String path) {
			...
			try {
				...
				final File outputFile = TextUtils.isEmpty(path)
					...
					: new File(parent.getExternalFilesDir("captured"), path);
				if (DEBUG) Log.v(TAG_THREAD, "path:"+outputFile.getAbsolutePath());
		...
		public void handleStartRecording(String prefix) {
			if (DEBUG) Log.v(TAG_THREAD, "handleStartRecording:");
			try {
				if ((mUVCCamera == null) || (mMuxer != null)) return;
				final MediaMuxerWrapper muxer = new MediaMuxerWrapper(prefix+".mp4");	// if you record audio only, ".m4a" is also OK.
                ...
                //changed due to double callbacks
				//if (videoEncoder != null) {
		...
		public void handleStopRecording() {
			...
			if (muxer != null) {
                if (DEBUG) Log.v(TAG_THREAD, "stop muxer");
				muxer.stopRecording();
                //do not release the callback here
				//mUVCCamera.setFrameCallback(null, 0);
				// you should not wait here
				//callOnStopRecording();
                //do this in medialistener on stopped
			}

		}

		public void handleFrameCallback(boolean add) {
			if (!isRecording() && mUVCCamera != null) {
				if (add && mIFrameCallback != null) {
